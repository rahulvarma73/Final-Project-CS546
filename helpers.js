...
const validpassword = (password) => {
  ...
  if (password.length < 6)
    throw `400passwords must be at least 6 characters long!`;
  if (!/[`!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/.test(password))
    throw `400passwords must have atleast one special character!`;
  if (!/[A-Z]/.test(password))
    throw `400passwords must have atleast one capital alphabet!`;
  if (!/[0-9]/.test(password))
    throw `400passwords must have atleast one number!`;
  return password;
};
...
const validname = (name) => {
  ...
  if (!/[a-zA-Z]+/.test(name))
    throw `400Names should have at least one alphabet each!`;
  return name;
};
